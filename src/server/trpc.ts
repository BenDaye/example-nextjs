...
import { CommonTRPCError } from './errors';
...
const isAuthorized = t.middleware(async ({ ctx, next }) => {
  if (
    ...
    (ctx.session?.user?.role !== UserRole.ADMIN &&
      ctx.session?.user?.role !== UserRole.USER)
  )
...
const isAuthorizedUser = t.middleware(async ({ ctx, next }) => {
  if (!ctx.session?.user?.id || ctx.session?.user?.role !== UserRole.USER)
    throw new CommonTRPCError('UNAUTHORIZED');
...
const isAuthorizedAdmin = t.middleware(async ({ ctx, next }) => {
  if (!ctx.session?.user?.id || ctx.session?.user?.role !== UserRole.ADMIN)
    throw new CommonTRPCError('UNAUTHORIZED');
